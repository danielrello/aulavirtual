...
def module001_course():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            if not form.id.data:
                ...
                course.code = 'C' + str(course.id) + ''.join(
                    random.choice('AILNOQVBCDEFGHJKMPRSTUXZ') for i in range(4))
        ...
        if not course or course.user_id != current_user.id:
            ...
            form = CourseForm(id=course.id, name=course.name.replace('-' + course.code, ''),
                              institution_name=course.institution_name, code=course.code)
...
def module001_sharing_details():
    ...
    if request.args.get('itemtype') == 'course':
        ...
        if not course or course.user_id != current_user.id:
            ...
            return redirect(url_for('module001.module001_course'))
        qr.add_data("http://{}/course/follows?sharedlink=1&code={}".format(base_url, course.code))
    ...
    try:
        ...
        im.save(filename)
        return render_template('module001_sharing_details.html', module="module001", item=item, itemtype=itemtype,
                               filename=urlfilename, base_url=base_url)
    except:
        return render_template('module001_sharing_details.html', module="module001", item=item, itemtype=itemtype,
                               base_url=base_url)


@module001.route('/follows', methods=['GET', 'POST'])
@login_required
    ...
    if request.method == 'POST':
        try:
            if not form.id.data:
                ...
                while (True):
                    participation_code = 'P' + str(form.course_id.data) + ''.join(
                        random.choice('AILNOQVBCDEFGHJKMPRSTUXZ') for i in range(4))
                ...
                if course.institution_name != newname:
                    ...
                    db.session.query(ParticipationRedeem).filter(
                        ParticipationRedeem.code_description == oldname).update(
                        {ParticipationRedeem.code_description: newname}, synchronize_session=False)
                ...
                if course:
                    ...
                    if form.never_expire.data:
                        ...
                        participation.date_expire = datetime.datetime.combine(form.date_expire.data,
...
def module001_participation_redeem():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if not follow:
                flash(
                    'You are not following this course. Ask the course code for the professor and follows the course before redeeming any participation.')
            else:
    ...
    return render_template('module001_participation_redeem.html', module="participation_redeem", form=form,
                           rows=participations, unfollow=unfollow)
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if redeem:
                ...
                db.session.commit()
                flash(
                    "Code {} has been removed, you can always redeem again if you are still within the deadline".format(
                        participation_code))
                return redirect(url_for('module001.module001_participation_redeem'))
    else:
        redeem = ParticipationRedeem.query.filter(
            ParticipationRedeem.participation_code == request.args.get('participation_code'),
        ...
        if redeem:
            ...
            db.session.commit()
            flash(
                "Code {} has been removed from user {}, user can always redeem again if still within the deadline (extending the deadlines works)".format(
                    participation_code, request.args.get('used_id')))
            return redirect(url_for('module001.module001_participation_generate'))
